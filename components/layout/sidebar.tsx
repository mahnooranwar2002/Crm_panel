  ...
  return (
    ...
    >
      ...
      <div className="h-[70px] flex items-center justify-between px-4 mb-4">
        {!isCollapsed && (
          <div className="flex ml-10 items-center gap-3 pl-2">
            <Image src={logo} alt="Wholcure Logo" width={120} height={40} className="object-contain" />
          </div>
        )}
        <button onClick={toggle} className={cn("p-2 hover:bg-slate-50 rounded-xl transition-all text-slate-400 hover:text-[#21a9ff]", isCollapsed ? "mx-auto" : "")}>
      ...
      <nav className="flex-1 px-3 space-y-1.5 overflow-y-auto custom-scrollbar">
        {menuItems.map((item, index) => {
          ...
          const isDropdownOpen = openDropdown === item.label;
          const isSubmenuActive = item.submenu?.some((sub: any) => pathname.startsWith(sub.href));
          ...
          return (
            <div key={index} className="relative">
              ...
              <div
                onClick={() => hasSubmenu && setOpenDropdown(isDropdownOpen ? null : item.label)}
                className={cn(
                  "flex items-center p-3.5 rounded-xl cursor-pointer transition-all duration-200 group relative",
                  isActive 
                    ? "bg-[#21a9ff] text-[#fff] shadow-sm shadow-indigo-50/50" 
              ...
              >
                ...
                <Link href={item.href} className="flex items-center flex-1 min-w-0">
                  <item.icon className={cn("text-xl min-w-[20px] transition-transform group-hover:scale-110", isActive ? "text-white" : "text-slate-400 group-hover:text-[#21a9ff]")} />
                  {!isCollapsed && (
                    <span className={cn("ml-3 text-[14px] font-semibold tracking-wide truncate", isActive ? "text-[#fff]" : "text-slate-600")}>
                      {item.label}
                    </span>
                  )}
                </Link>
                ...
                {!isCollapsed && hasSubmenu && (
                  <FiChevronDown className={cn("text-lg transition-transform", isDropdownOpen ? "rotate-180" : "", isActive ? "text-white" : "text-slate-400")} />
                )}
              </div>

              {/* Submenu Content */}
              {!isCollapsed && hasSubmenu && isDropdownOpen && (
                <div className="mt-1 space-y-1 border-l-2 border-slate-100 ml-8 transition-all duration-300">
                  {item.submenu?.map((subitem: any, subindex: number) => {
                    const isSubActive = pathname === subitem.href;
                    return (
                      <Link key={subindex} href={subitem.href}>
                        <div className={cn(
                          "flex items-center p-2.5 rounded-lg cursor-pointer transition-all text-sm ml-2",
                          isSubActive 
                            ? "text-[#21a9ff] font-bold bg-[#21a9ff]/5" 
                            : "text-slate-500 hover:text-[#21a9ff] hover:bg-slate-50"
                        )}>
                          {subitem.label}
                        </div>
                      </Link>
